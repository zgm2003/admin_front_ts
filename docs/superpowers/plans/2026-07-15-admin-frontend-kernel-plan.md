...
- [x] **Step 3: Apply only mechanical lint fixes**
...
- [x] **Step 4: Record the ratcheting warning gate**
...
- [x] **Step 5: Verify and commit infrastructure**
...
- [x] **Step 1: Write tamper and stale-generation tests**
...
- [x] **Step 2: Define the exact lock**
...
- [x] **Step 2: Parse a closed Admin environment**
...
- [x] **Step 3: Implement the kernel state machine**
...
- [x] **Step 4: Mount only the neutral shell before bootstrap**
...
- [x] **Step 1: Test the complete state machine**
...
- [x] **Step 2: Define read-only session state**
...
- [x] **Step 3: Implement one refresh flight**
...
- [x] **Step 4: Implement ordered logout**
...
- [x] **Step 1: Test timeout, cancel, replay, and contract failure**
...
- [x] **Step 2: Define closed errors and operation policy**
...
- [x] **Step 4: Provide a temporary typed compatibility facade**
...
- [x] **Step 1: Generate a literal local view allowlist**
...
- [x] **Step 2: Test invalid and repeated sets**
...
- [x] **Step 3: Define closed route contracts**
...
- [x] **Step 4: Make guards read kernel/session only**
...
- [x] **Step 1: Test corruption, versioning, limits, and logout**
...
- [x] **Step 2: Define codecs and namespaces**
...
- [x] **Step 3: Migrate allowed values**
...
- [x] **Step 1: Write behavior tests before migration**
...
- [x] **Step 2: Replace page-owned authentication**
...
- [x] **Step 3: Remove legacy credential dependencies**
...
- [x] **Step 1: Guard the Docker-only boundary**
...
- [x] **Step 2: Implement the shared source and image verifier**
...
- [x] **Step 3: Keep Compose as the only runtime entry**
...
- Tasks 1–9 were executed directly on frontend `master` by explicit operator instruction; no secondary worktree was created or retained.
- AppKernel, AuthSession, typed ApiClient/ApiError, atomic route replacement, generated contract types, versioned device/user persistence, and kernel-owned login/logout are committed through `c4247cd`.
- Docker-only delivery is implemented by `84868a6`; both repositories contain no `.github` or `.worktrees` directory, and each registers only its primary `master` checkout.
- `scripts/verify-frontend.ps1` exited `0` from a clean checkout. Contract and route generation were clean, lint held the 82-warning ratchet with zero errors, typecheck passed, and 450 unit/component/integration tests passed with 87.94% statements and 81.72% branches.
- The production build ran only inside `Dockerfile`; the resulting image was inspected for the exact Git revision label, unprivileged user `101`, `/healthz` healthcheck, and `8080/tcp` exposure.
- `admin_back_go/scripts/docker-platform.ps1 up` rebuilt and started the frontend, API, and worker images. All five Compose containers became healthy; frontend `/healthz`, API `/health`, and API `/ready` returned success without host Vite or Go application processes.
