...
const routes: any[] = [
  ...
  { path: '/:pathMatch(.*)*', name: 'NotFound', beforeEnter: (_to: any, _from: any, next: any) => { next(false) } },
]
...
router.afterEach((to) => { const menuStore = useMenuStore(); menuStore.selectedMenu = to.meta.menuId; Cookies.set('selectedMenu', to.meta.menuId as any) })
