...
const searchFields = computed(() => [
  ...
  { key: 'status', type: 'select-v2' as const, label: '状态', placeholder: '请选择', width: 150, options: [{ value: 1, label: '启用' }, { value: 0, label: '禁用' }] }
])
const handleQuery = (form: any) => { console.log('查询:', form) }
...
const handleIconSelect = (name: string) => { selectedIcon.value = name }

const searchProps = [
  { name: 'v-model / modelValue', type: 'Object', default: '{}', desc: '表单数据对象，支持双向绑定' },
  { name: 'fields', type: 'Field[]', default: '[]', desc: '字段配置数组，每项包含 key/type/label/placeholder/width/options/props' },
    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="Search 搜索" name="Search">
        <div class="demo-section">
          ...
          <div class="demo-code">
            <el-text type="info">&lt;Search v-model="searchForm" :fields="searchFields" @query="handleQuery" /&gt;</el-text>
          </div>
        </div>
        <div class="demo-section">
          <h4>Attributes</h4>
        ...
        <div class="demo-section">
          <h4>Field 字段类型</h4>
          <el-table :data="[
            { type: 'input', desc: '文本输入框' },
            { type: 'select-v2', desc: '虚拟下拉选择器，需配置 options' },
            { type: 'cascader', desc: '级联选择器，需配置 options' },
            { type: 'date-range', desc: '日期范围选择器' }
          ]" border>
          ...
          <div class="demo-block">
            <el-space>
              ...
              <el-tag v-if="selectedIcon">
                <el-icon><component :is="selectedIcon" /></el-icon>
                {{ selectedIcon }}
              </el-tag>
            </el-space>
