...
const init = () => { initApi().then((data:any)=>{ PermissionTree.value = data.dict.permission_tree }).catch(()=>{}) }
init()
const addBoxShow = ref(false)
const addForm = ref({ name:'', permission_id:'' })
const add = () => { addForm.value = { name:'', permission_id:'' }; addBoxShow.value = true }
const confirmAdd = () => { const param = addForm.value; addApi(param).then(()=>{ ElNotification.success({ message:'新增成功' }); addBoxShow.value = false; getList() }).catch(()=>{}) }
const listLoading = ref(false)
const listData = ref([])
const searchForm = ref({ name:'', path:'', permission:'' })
const page = ref({ current_page:1, page_size:50, total:0 })
const columns = [ { key:'id', label:t('role.table.id'), width:55 }, { key:'name', label:t('role.table.name') }, { key:'created_at', label:t('role.table.created_at') }, { key:'updated_at', label:t('role.table.updated_at') }, { key:'actions', label:'操作', width:180, align:'center' } ]
const getList = () => { listLoading.value = true; const param:any = { ...searchForm.value, page_size: page.value.page_size, current_page: page.value.current_page }; listApi(param).then((data:any)=>{ listLoading.value = false; listData.value = data.list; page.value = data.page }).catch(()=>{ listLoading.value = false }) }
getList()
    ...
    <el-form :inline="true" :model="searchForm">
      <el-form-item>
        <el-dropdown>
          ...
          <template #dropdown><el-dropdown-menu><el-dropdown-item @click="batchDel" v-if="userStore.can('role.del')">{{ t('common.actions.batchDelete') }}</el-dropdown-item></el-dropdown-menu></template>
        </el-dropdown>
      </el-form-item>
      <el-form-item v-if="userStore.can('role.add')"><el-button type="success" @click="add">{{ t('common.actions.add') }}</el-button></el-form-item>
      <el-form-item><el-input v-model="searchForm.name" clearable :placeholder="t('role.filter.name')" style="width:150px" /></el-form-item>
  ...
  <el-dialog v-model="addBoxShow" class="add-box dialog-box" width="800">
    ...
    <div class="content-box">
      <el-form :model="addForm" label-width="auto">
        <el-form-item label="角色名" required><el-input v-model="addForm.name" clearable style="width:100%" /></el-form-item>
        <el-form-item label="权限"><el-cascader :options="PermissionTree" :props="props" v-model="addForm.permission_id" collapse-tags clearable style="width:100%" /></el-form-item>
      </el-form>
    </div>
  ...
  <el-dialog v-model="editBoxShow" class="add-box" width="1000" title="编辑" top="20vh">
    <div class="add-box">
      <el-form label-width="auto" :model="editForm">
        <el-form-item label="角色名" required><el-input v-model="editForm.name" clearable /></el-form-item>
        <el-form-item label="权限"><el-cascader :options="PermissionTree" :props="props" v-model="editForm.permission_id" clearable style="width:100%" /></el-form-item>
      </el-form>
    </div>
