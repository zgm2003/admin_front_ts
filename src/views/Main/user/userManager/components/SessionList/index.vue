...
import type { UserSessionItem, UserSessionStats } from '@/types/user'

const { t } = useI18n()
const userStore = useUserStore()

const platformArr = ref<DictOption<string>[]>([])
const stats = ref<UserSessionStats>({
  total_active: 0,
  platform_distribution: { admin: 0, app: 0 },
})
...
const statusOptions = computed<DictOption<'active' | 'expired' | 'revoked'>[]>(() => [
...
const confirmKick = async (row: UserSessionItem) => {
  try {
    await ElMessageBox.confirm(t('userSession.confirmKick'), t('common.confirmTitle'), {
      type: 'warning',
      confirmButtonText: t('common.actions.kick'),
...
const batchKick = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      t('userSession.confirmBatchKick', { count: selectedIds.value.length }),
