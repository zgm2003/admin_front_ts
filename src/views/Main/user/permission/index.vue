...
const getList = () => { listLoading.value = true; const param = searchForm.value; listApi(param).then((data:any)=>{ listLoading.value = false; listData.value = data }).catch(()=>{ listLoading.value = false }) }
getList()
const tableRef = ref()
...
const openEditIconSelect = () => { iconEditSelect.value.show() }
const confirmEditIcon = (iconName:string) => { editForm.value.icon = iconName }
const changStatus = (row:any) => { if (row.status === undefined) { ElNotification.error({ message:'启用状态字段不存在' }); return } const param = { id: row.id, status: row.status }; statusApi(param).then(()=>{ ElNotification.success({ message:'修改成功' }); getList() }).catch(()=>{}) }
</script>
...
<template>
  <div class="box" style="margin-left:10px;">
    <el-form :inline="true" :model="searchForm">
      <el-form-item>
        <el-dropdown>
          <el-button type="primary">{{ t('common.actions.batchDelete') }}<el-icon class="el-icon--right"><arrow-right/></el-icon></el-button>
          <template #dropdown><el-dropdown-menu><el-dropdown-item @click="batchDel" v-if="userStore.can('permission.del')">{{ t('common.actions.batchDelete') }}</el-dropdown-item></el-dropdown-menu></template>
        </el-dropdown>
      </el-form-item>
      <el-form-item v-if="userStore.can('permission.add')"><el-button type="success" @click="add">{{ t('common.actions.add') }}</el-button></el-form-item>
      <el-form-item><el-input v-model="searchForm.name" clearable :placeholder="t('permission.filter.name')" style="width:150px" /></el-form-item>
      <el-form-item><el-button type="primary" @click="getList">{{ t('common.actions.query') }}</el-button></el-form-item>
    </el-form>
    <div class="table">
      <el-table :data="listData" style="width:100%" v-loading="listLoading" @selection-change="handleSelectionChange" ref="tableRef" @row-click="handleRowClick" row-key="id" border default-expand-all>
        ...
        <el-table-column label="ICON" align="center"><template #default="scope"><el-icon><component :is="scope.row.icon" /></el-icon></template></el-table-column>
  ...
  <el-dialog v-model="addBoxShow" class="add-box dialog-box" width="800" draggable>
    ...
    <div class="content-box">
      <el-form :model="addForm" label-width="auto">
        ...
        <el-form-item label="component" v-if="addForm.type === 2"><el-input v-model="addForm.component" style="width:100%" clearable type="textarea" :rows="5" /></el-form-item>
        <el-form-item label="code" required v-if="addForm.type === 3"><el-input v-model="addForm.code" style="width:100%" clearable /></el-form-item>
      </el-form>
    </div>
    <template #footer><span class="dialog-footer"><el-button @click="addBoxShow=false">取消</el-button><el-button type="primary" @click="confirmAdd">确认</el-button></span></template>
  </el-dialog>
  <el-dialog v-model="editBoxShow" class="add-box" width="800" title="编辑" top="20vh" draggable>
    <div class="add-box">
      <el-form label-width="auto" :model="editForm">
        ...
        <el-form-item label="component" v-if="editForm.type === 2"><el-input v-model="editForm.component" style="width:100%" clearable type="textarea" :rows="5" /></el-form-item>
        <el-form-item label="code" required v-if="editForm.type === 3"><el-input v-model="editForm.code" style="width:100%" clearable /></el-form-item>
      </el-form>
    </div>
