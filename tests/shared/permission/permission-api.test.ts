...
describe('permission api REST contract', () => {
  ...
  it('declares the new API base URL as the only active request client', () => {
    ...
    expect(envProduction).toMatch(/^VITE_GO_API_BASE_URL=https:\/\/api\.example\.com$/m)
    expect(envProduction).toMatch(/^VITE_WEB_SOCKET_URL=wss:\/\/admin\.example\.com\/api\/admin\/v1\/realtime\/ws$/m)
    expect(envProduction).not.toContain('zgm2003.cn')
  })
})
