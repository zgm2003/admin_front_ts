...
describe('auth platform api REST contract', () => {
  it('uses default REST endpoints instead of legacy all-post auth platform routes', () => {
    ...
    expect(source).toContain('const pageInit = () => request.get<AuthPlatformInitResponse>(`${ADMIN_API_PREFIX}/auth-platforms/page-init`)')
    ...
    expect(source).toContain('request.delete<void>(`${ADMIN_API_PREFIX}/auth-platforms/${normalizeAuthPlatformIDs(params.id)[0]}`)')
