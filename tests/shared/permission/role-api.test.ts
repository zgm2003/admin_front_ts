...
describe('role api REST contract', () => {
  it('uses default REST role endpoints instead of legacy all-post role routes', () => {
    ...
    expect(source).toContain('const pageInit = () => request.get<RoleInitResponse>(`${ADMIN_API_PREFIX}/roles/page-init`)')
    ...
    expect(source).toContain('request.delete<void>(`${ADMIN_API_PREFIX}/roles/${normalizeRoleIDs(params.id)[0]}`)')
