...
describe('role api REST contract', () => {
  it('uses default REST role endpoints instead of legacy all-post role routes', () => {
    ...
    expect(source).toContain("request.get<RoleInitResponse>('/api/v1/roles/init')")
    expect(source).toContain("request.get<PaginatedResponse<RoleListItem>>('/api/v1/roles'")
    expect(source).toContain("request.post<void, RoleAddPayload>('/api/v1/roles'")
    expect(source).toContain('request.put<void, RoleAddPayload>(')
    expect(source).toContain('request.patch<void>(`/api/v1/roles/${params.id}/default`)')
    expect(source).toContain('request.delete<void>(`/api/v1/roles/${ids[0]}`)')
    expect(source).toContain("request.delete<void, RoleBatchDeletePayload>('/api/v1/roles'")
