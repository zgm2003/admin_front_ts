...
describe('AI tools api contract', () => {
  ...
  it('exposes standard tool wrapper names with legacy aliases', () => {
    const source = readFrontendSource('src/api/ai/tools.ts')
    expect(source).toContain('const pageInit = () => request.get<AiToolInitResponse>')
    expect(source).toContain('const generatePageInit = () => request.get<AiToolGenerateInitResponse>')
    expect(source).toContain('const create = (params: AiToolMutationParams)')
    expect(source).toContain('const update = (params: AiToolMutationParams)')
