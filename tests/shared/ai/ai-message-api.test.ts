...
describe('AI message api minimal REST contract', () => {
  it('uses only conversation-scoped list and send endpoints', () => {
    ...
    expect(source).toContain('meta_json?: AiMessageMeta')
    expect(source).toContain('attachments?: AiChatAttachment[]')
    expect(source).toContain('runtime_params?: Record<string, number>')
