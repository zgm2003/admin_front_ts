...
describe('queue monitor api and page contract', () => {
  it('uses the authenticated official asynqmon mount instead of legacy queue monitor routes', () => {
    ...
    expect(source).toContain("import { service } from '@/lib/http'")
    expect(source).toContain("import { getCommonHeaders } from '@/lib/http/headers'")
    ...
    expect(source).toContain('ensureQueueMonitorAuthCookie')
    expect(source).toContain('service.head(ADMIN_QUEUE_MONITOR_UI_PATH')
