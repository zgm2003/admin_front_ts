...
describe('export task api REST contract', () => {
  it('uses Go REST endpoints instead of legacy export task routes', () => {
    ...
    expect(source).toContain('request.delete<void>(`${ADMIN_API_PREFIX}/export-tasks/${ids[0]}`)')
    expect(source).toContain('request.delete<void, { ids: number[] }>(`${ADMIN_API_PREFIX}/export-tasks`, { data: { ids } })')
